...
- **Advisory method classifier row** in `animedex/transport/read_only.py` if the backend is a new entry, with the known read method/path rules spelled out. This classifier is informational; it must not block raw passthrough calls.
...
- Affirmation is welcome ("§15 advisory classifier row added correctly") but not required; silence on a check is implicit acknowledgement that it passed.
