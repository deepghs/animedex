...
- **Read-only high-level scope**: animedex does not implement high-level `PATCH/POST/DELETE` helpers against user-account mutation endpoints. The raw `animedex api` passthrough is the deliberate escape hatch and does not reject methods on the user's behalf. If a caller intentionally sends `POST`, `PUT`, `PATCH`, or `DELETE` through the raw passthrough, animedex forwards it and returns the upstream result. High-level commands stay read-oriented; raw passthrough commands respect caller intent.
...
- **Read-oriented high-level API surface coverage** — every anonymous read endpoint the upstream documents must be exposed as a high-level helper and a CLI command, not a representative subset. Where the upstream offers an authenticated read tier (no mutation), it is also exposed, gated on credential resolution. High-level mutation endpoints (`POST` / `PATCH` / `DELETE` outside of GraphQL or known read-with-body paths) are excluded by section 6. The wider the read surface, the better Codex / Claude can compose calls; partial surfaces force agents to fall back to the raw passthrough and lose typed-result benefits. Treat "I shipped only what looked useful" as a smell.
...
- **Advisory method classifier row** in `animedex/transport/read_only.py` if the backend is a new entry, with the known read method/path rules spelled out. Anonymous-read backends typically mark `GET` as known-read and `POST` as not-known-read; GraphQL backends or specific read-with-body endpoints carve out the narrow known-read `POST`. This classifier is informational only - it drives cache eligibility but does not block raw passthrough calls.
...
- Affirmation is welcome ("§15.2 advisory method classifier row added correctly") but not required; silence on a check is implicit acknowledgement that it passed.
